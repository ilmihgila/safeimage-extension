...
function textContentFromPrediction(predictions) {
    ...
    // Confident.
    if (predictions[0].className == 'nude') {
        return `Konten dibatasi`;
    }
    if (predictions[0].className == 'safe') {
        return `gambar baik`;
    }
}
/**
...
function detectImages(sendResponse) {
    ...
    sendResponse({ srcUrls }); // Send the srcUrls array as a response.
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message.action) {
        case 'DETECT_IMAGES':
            console.log('ini beneran ke console gak sih?')
            detectImages(sendResponse);
            return true;
        case 'DA_IMAGUS_BRADER':
            console.log('jalan imagusnya');
            loadImageAndSendDataBack(message.url, sendResponse);
            return true;
        case 'IMAGE_CLICK_PROCESSED':
            if (message.url && message.predictions) {
                console.log('jalan prosesnya');
                ...
                for (const imgNode of imgElements) {
                    ...
                    if (textContent === 'Konten dibatasi') {
